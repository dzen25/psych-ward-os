...
- **`load_chain_test/`** (корень репозитория) — уже собранные файлы цепочки загрузки, готовые к записи на SD-карту (см. [Фазу 1.4](#14-собрать-sd-карту)).
- На хост-машине (вне репозитория) заведена рабочая папка `RPi4_SeL4/` с исходниками цепочки загрузки: `load_chain/` (сырые файлы до копирования в репозиторий), `u-boot/` (клон и сборка U-Boot), `sel4-rpi4-hello/` (отдельное дерево sel4test для проверки hello-world, **не** `psych-ward-os`).
- **`load_chain/`** (корень репозитория) — собранные готовые файлы цепочки загрузки для загрузки `psych-ward-os` на SD-карту.
...
Приветствуются новые идеи и форки. Базовая архитектура Psych Ward OS (не связанная с портом) расписана в README ветки `main` и в [CONTRIBUTING.md](CONTRIBUTING.md).

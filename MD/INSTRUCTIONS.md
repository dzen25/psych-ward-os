...
sudo diskutil partitionDisk /dev/diskN MBR FAT32 BOOT 2GiB ExFAT RPI R
sudo diskutil unmount /dev/diskNs1
sudo newfs_msdos -F 32 -c 32 -v BOOT /dev/rdiskNs1
sudo diskutil mount /dev/diskNs1
```

Проверить, что получилось: `diskutil info /dev/diskNs1` — строка
`Allocation Block Size` должна показать **16384 Bytes**.
...
sudo mkfs.vfat -F 32 -s 32 -n BOOT /dev/sdX1   # -s 32 = 32 сектора = 16 КиБ кластер, см. врезку выше
sudo mkfs.exfat -n RPI /dev/sdX2   # нужен exfatprogs (apt install exfatprogs)
```

У `mkfs.vfat` размер кластера задаётся флагом `-s` (тоже в секторах), в
отличие от `-c` у `newfs_msdos`.


⚠️ **Размер кластера на BOOT определяет время загрузки — это не мелочь.**
Подтверждено на живом железе 2026-09-11:

| разметка BOOT | кластер | блочных чтений на `boot.itb` | время `fatload` |
|---|---|---|---|
| `128MiB` (было) | 512 Б | 139 | 526 мс (3.8 МБ/с) |
| `2GiB` (сейчас) | 16 КиБ | **2** | **107 мс (18.7 МБ/с)** |

Полсекунды с каждой загрузки. Причина в фрагментации: при кластере в один
сектор образ в 2 МБ раскладывался на 139 кусков, и U-Boot читал его 139
отдельными блочными командами вместо одной. Сам транспорт был ни при чём —
те же 2 МБ сырым `mmc read` читаются за 95 мс и при старой разметке тоже,
так что 107 мс это уже практически пол.

**Почему нельзя было просто задать крупный кластер на 128 МиБ.** FAT32 по
спецификации требует не меньше 65525 кластеров, отсюда жёсткое
соотношение:

```
размер кластера <= размер раздела / 65525
```

На 128 МиБ это даёт потолок около 2 КиБ — крупнее физически нельзя, и
`newfs_msdos` в такой тесноте уходила в минимум. То есть увеличивать нужно
было именно РАЗДЕЛ, а не только флаг.

2 ГиБ при кластере 16 КиБ дают ~131000 кластеров — вдвое больше минимума,
без игры на границе. Кластер 32 КиБ брать не стоит: на 2 ГиБ он даёт ровно
65536 кластеров ДО вычета служебных секторов, после вычета проваливается
под минимум, и `newfs_msdos` либо откажет, либо молча сделает FAT16. Для
32 КиБ понадобился бы раздел около 4 ГиБ, а выигрыша поверх 16 КиБ уже
почти нет.

Тип раздела в MBR остаётся `DOS_FAT_32`, прошивка RPi и U-Boot читают как
обычно. Место не пропадает: `R` в конце команды отдаёт весь остаток
разделу `RPI`, он просто становится 27 ГиБ вместо 29.

Две детали, на которых легко споткнуться: **`-c` считается в секторах**
(`-c 32` = 32 × 512 = 16 КиБ), и работать надо с **`/dev/rdiskNs1`** через
`r` — это сырое устройство, `newfs_msdos` на порядок быстрее с ним.

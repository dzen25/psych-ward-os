...
// USB-клавиатура -> консоль (uart_driver). Нажатия с USB HID-клавиатуры
// кладутся в ТОТ ЖЕ кольцевой буфер g_kbd_buffer, куда попадают байты с
// UART RX, — шелл и любая программа, читающая stdin, не отличают
// клавиатуру от набора в minicom.
//
// SYS_KBD_INJECT — байты в MR1..MRn. console_ep есть у каждого процесса
// (для SYS_PUTS), поэтому без проверки любая утилита из /sbin могла бы
// "напечатать" в админский шелл команду от имени пользователя. uart_driver
// принимает INJECT только от PID, назначенного источником клавиатуры.
//
// SYS_KBD_SET_SOURCE (MR1 = PID) — назначить этот источник. ТОЛЬКО ОТ
// ROOT: uart_driver принимает команду лишь с badge 0, а badge 0 есть
// только у неминченой копии console_ep, которую держит root; всем
// остальным процессам копия минтится с badge = их PID >= 1. Подделать
// это нельзя.
//
// Почему не "usb_driver регистрируется сам, кто первый — тот и источник"
// (так было в первой версии): PID выдаётся НАИМЕНЬШИЙ СВОБОДНЫЙ и при
// респавне меняется ("New PID" в логе watchdog'а), а у usb_driver включён
// auto_restart. После первого же респавна новый usb_driver был бы
// отвергнут — клавиатура мертва до перезагрузки, — а его прежний PID мог
// достаться пользовательской программе вместе с правом печатать в шелл.
// Теперь root назначает источник после каждого (ре)спавна usb_driver, а
// при смерти процесса-источника uart_driver снимает назначение сам (по
// SYS_CANCEL_PENDING_FOR_PID, который root и так шлёт ему до уборки).
constexpr seL4_Word SYS_KBD_SET_SOURCE = 155;
constexpr seL4_Word SYS_KBD_INJECT     = 156;

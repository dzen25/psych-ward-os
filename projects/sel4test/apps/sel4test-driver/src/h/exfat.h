...
struct DirCursor {
    ...
    char sector_buf[512];
    bool sector_loaded;
    // Сектор изменён в памяти и ещё не сброшен на носитель. Появился, чтобы
    // набор записей каталога (0x85 + 0xC0 + 0xC1...) писался ОДНИМ сбросом
    // сектора, а не отдельным чтением-записью на каждую 32-байтную запись:
    // все записи набора почти всегда лежат в одном секторе, а стоило это
    // 6 блочных команд вместо 2 — на каждое дописывание в файл.
    bool dirty;

...
# нужны оба.
#
# ПАТЧЕЙ ПРО СКОРОСТЬ ЧТЕНИЯ SD ЗДЕСЬ НЕТ И НЕ НУЖНО (issuse.txt №8,
# закрыт на железе 2026-09-11). U-Boot читал boot.itb на 3.8 МБ/с против
# 21 у нашего драйвера, и обе напрашивавшиеся версии оказались неверны:
#   * не режим шины — `mmc info` показал "SD High Speed (50MHz),
#     Bus Width: 4-bit", а caps=0x45ee6432, то есть бит 21
#     (SDHCI_CAN_DO_HISPD) стоял всегда. Патч, объявлявший режим явно,
#     был чистым no-op и удалён;
#   * не транспорт вообще — те же 2 МБ сырым `mmc read` шли 95 мс
#     (21.8 МБ/с), то есть ровно на скорости нашего драйвера.
# Виновата оказалась РАЗМЕТКА КАРТЫ: раздел BOOT был отформатирован с
# кластером в 512 байт, файл в 2 МБ ложился 139 фрагментами, и U-Boot
# делал 139 блочных чтений вместо одного. После переформатирования с
# кластером 16 КиБ — 2 чтения и 107 мс вместо 526. Команда разметки и
# соотношение "кластер <= размер раздела / 65525" — в
# MD/INSTRUCTIONS.md, раздел про подготовку SD-карты. ---
(
    cd "$UBOOT_DIR"
...
(
    ...
    # CMD_TIME оставлен намеренно, хотя разбирательство по issuse.txt №8
    # закончено: `time <команда>` в приглашении U-Boot — самый дешёвый
    # способ отделить "медленно читает карта" от "медленно работает
    # файловая система", а именно на этом разборе версия про режим шины
    # и умерла. В rpi_4_defconfig его нет, стоит несколько килобайт.

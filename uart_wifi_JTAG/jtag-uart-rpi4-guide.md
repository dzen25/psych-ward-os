...
> **2026-08-23**: реальный FT232H сейчас подключен к выделенному Linux-хосту `carto`, не к Mac — актуальный, проверенный вживую статус см. в `debug_of_FT232H/FT232H-jtag-guide.md`. Распиновка (§2) и `config.txt` (§3) ниже по-прежнему верны (не зависят от хоста), остальное (установка/запуск/питфолы) — устарело относительно текущего стенда, оставлено как справка на случай возврата к Mac.

> **2026-09-06**: появилась ветка `load_over_JTAG/` — не отладка, а **загрузка образа Psych Ward OS прямо в ОЗУ через JTAG**, минуя перепрошивку SD-карты. Стоит поверх стека FT232H (те же провода, тот же OpenOCD на carto), ничего в нём не меняет. См. `load_over_JTAG/JTAG-load-guide.md`.

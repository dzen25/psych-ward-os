...
- [х] Декомпилировать `bcm2711-rpi-4-b.dtb`
- [x] Определить и указать ниже, какие узлы будут нужны в целом (не только genet/EMMC2 — см. также раздел «Что нужно портировать» в README)
- [x] Найти узел `genet` (адрес, IRQ)
- [x] Найти узел EMMC2 (адрес, IRQ)
- [x] Занести найденные адреса в отдельную rpi4-секцию `platform.h`
</details>
...
- [ ] **Фаза 2 — Извлечение адресов под свои драйверы**
  - [x] Декомпилировать `bcm2711-rpi-4-b.dtb` → `.dts` (`dtc`)
  - [x] Определить, какие узлы будут необходимы в целом
  - [x] Найти узел `genet` (адрес, IRQ)
  - [x] Найти узел EMMC2 (адрес, IRQ)
  - [x] Занести найденные адреса в отдельную rpi4-секцию `platform.h`
